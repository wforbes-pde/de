...
def MLP_model(df):
    ...
    if False:
        ...
        plt.savefig(f'images/houston-lmp-mlp.png',
                    format='png',
                    dpi=300,
                    bbox_inches='tight')
        plt.show()
...
# DE parameters best rmse 75

g = 400 # max number of generations 400
NP = 400 # 400
F = 0.5 # 0.5
CR = 0.75 # 0.75
angle = 90
mutation_type = 'best' # best (50,1000,0.5,0.7), gen_best (same as best), random (rmse increases, stagnates even with large NP)
clustering_type = None # kmeans, spectral, agg. kmeans best so far? overall not very effective. 
num_of_clusters = 20 # 
cluster_gen_begin = 50 # 
rotate_gen_begin = 20 # 20
num_replace = 1 # 1
tol = -0.05
NPI = 1600 # 1600
F_scaling = 0.95 # 0.95
...
logging.info(f'Saving to {output_loc}')
with pd.ExcelWriter(output_loc ) as writer:
    dfs.to_excel(writer, sheet_name = 'final', index=False)
...
if True:
    ...
    plt.ylabel('LMP')
    plt.legend(fontsize = 10, loc='upper left')
    plt.savefig(f'images/houston-lmp-denn.png',
                format='png',
                dpi=300,
                bbox_inches='tight')
    plt.show()
